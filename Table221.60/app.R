...
# ==============================Arrange the state selection option=======================
all_rows<-c("Alabama"	=	"Alabama"	,
            "Alaska"	=	"Alaska"	,
            "Arizona"	=	"Arizona"	,
            "Arkansas"	=	"Arkansas"	,
...
server <- function(input, output,session) {
    ...
    leaflet(options = leafletOptions(zoomControl = FALSE)) %>%
      ...
      addPolygons(data=interim,
                  ...
                  popup = paste0(
                    ...
                    , "</br>"
                    ,"<strong>NAEP Reading Scale Score (SE): </strong>", interim$Score," (",interim$SE,")"
                  )
      ) %>%
      addLegend(position = "topleft", pal = mypal_nona, values = as.numeric(interim$Score),
                title = "NAEP Reading Scale Score </br> across U.S. Public Schools",
                opacity = 1)
  }
  )
    ...
    if (is.null(input$mymap_shape_click$id)==F){
      ...
      highchart() %>%
        ...
        hc_yAxis(title ="NAEP Reading Scores",
                 ...
                 min=235,
                 max=285) %>%
        hc_yAxis(title = list(text = "NAEP Reading Scale Scores")) %>%
        hc_add_series(name="NAEP Reading Scale Scores",
        ...
        hc_add_series(name="National NAEP Reading Scale Scores",
        ...
        hc_title(
          text = paste0( "The NAEP Reading Scale Scores of ", input$mymap_shape_click$id)
        )
    } else {
      ...
      hchart(df, "line", hcaes(x = year, y = as.numeric(Score), group=location, showInLegend = TRUE))%>%
        ...
        hc_yAxis(title ="NAEP Reading Scale Scores",
                 tickInterval=10,
                 min=235,
                 max=285) %>%
        hc_title(
          text = "The NAEP Reading Scale Scores of all states (including U.S National and Department of Defense Education Activity)"
        ) %>%
        hc_legend(
          align = "left",
  ...
  output$compare <- renderHighchart({
    ...
    list<-list(a=c("CT","MA","ME","NH","RI","VT","NJ","NY","PA"),
               b=c("IL","IN" ,"MI", "OH", "WI" ,"IA", "KS", "MN", "MO", "NE", "ND", "SD"),
